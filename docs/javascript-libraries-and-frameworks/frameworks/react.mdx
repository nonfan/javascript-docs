...
`useEffect` Hook 可以替代 `componentDidMount`、`componentDidUpdate` 和 `componentWillUnmount`。通过传递不同的依赖数组，可以控制 `useEffect` 的触发时机和条件。
...
Hooks 只能在 React 的函数式组件中使用，不能在普通的 JavaScript 函数中使用，也不能在类组件中使用。以及自定义 Hook 中调用 Hooks。
